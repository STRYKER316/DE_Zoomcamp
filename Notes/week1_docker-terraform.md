...
***Data Engineering*** is the design and development of systems for ingesting/collecting, storing and analyzing data at large-scale.

## Architecture

We will build the following architecture:

![architecture diagram](images/system_architecture.jpg)

* [New York's Taxi and Limousine Corporation's Trip Records Dataset](https://github.com/DataTalksClub/data-engineering-zoomcamp/blob/main/dataset.md): Source of datasets
* [NewYork Yellow Taxi Trip Records for Jan 2021](https://de-zoomcamp-files.s3.ap-south-1.amazonaws.com/yellow_tripdata_2021-01.csv): Actual Dataset that is used
...
A **data pipeline** is a service that receives data as input and outputs more data. For example, reading a CSV file, processing/transforming the data, and storing it as a table in a PostgreSQL database.
...
_([Video - Introduction to Docker](https://www.youtube.com/watch?v=EYNwNlOrpr0&list=PL3MmuxUbc_hJed7dXYoJw8DoCuVHhGEQb&index=3))_

**Docker** is a _containerization software_ that allows us to isolate software in a similar way to virtual machines but in a much leaner way.

A **Docker image** is a _snapshot_ of a container that we can define to run our software (We'll run a Data-pipeline, PostgreSql server, PgAdmin in docker). By exporting our Docker images to Cloud providers such as Amazon Web Services or Google Cloud Platform, we can also run our containers there.
...
# set up our image by installing prerequisites; pandas in this case (pandas required Pyarrow)
...
```ssh
docker build -t test-pipeline:pandas .
```
* The image name will be `test-pipeline` and its tag will be `pandas`. If the tag isn't specified it will default to `latest`.

We can now run the container and pass an argument to it, so that our pipeline will receive it:

```ssh
docker run -it --rm test-pipeline:pandas some_number
```
...
Install [pgcli](https://www.pgcli.com/) with the following command, if it is not already installed:
...
We will now create a Jupyter Notebook `upload_yellow_taxi_data.ipynb` file which we will use to read a CSV file and export it to Postgres.

We will use data from the [NYC TLC Trip Record Data website](https://www1.nyc.gov/site/tlc/about/tlc-trip-record-data.page). Specifically, we will use the [Yellow taxi trip records CSV file for January 2021](https://de-zoomcamp-files.s3.ap-south-1.amazonaws.com/yellow_tripdata_2021-01.csv). A dictionary to understand each field is available [here](https://www1.nyc.gov/assets/tlc/downloads/pdf/data_dictionary_trip_records_yellow.pdf).
...
* Click on Register.
* Click on Server.
* The remaining steps to create a server are the same as the screenshots.
...
_([Video04 - Converting Notebook to a script and putting the ingestion script into Docker](https://www.youtube.com/watch?v=B1WwATwf-vY&list=PL3MmuxUbc_hJed7dXYoJw8DoCuVHhGEQb&index=8))_

We will now convert the Jupyter notebook we used for data ingestion to a regular Python script and use a docker container to run it.
...
```bash
jupyter nbconvert --to=script upload_yellow_taxi_data.ipynb
```

Clean up the script by removing everything we don't need. We will also rename it to `ingest_yellow_taxi_data.py` and add a few modifications:
...
* The _engine_ we created for connecting to Postgres will be tweaked so that we pass the parameters and build the URL, as follows:
    ```python
    engine = create_engine(f'postgresql://{user}:{password}@{host}:{port}/{database_name}')
    ```
* We will also download the CSV using the provided URL argument.

You can find the completed `ingest_yellow_taxi_data.py` script [in this link](../week1_docker-terraform/2_docker-sql/ingest_yellow_taxi_data.py).

>```Note```: The PostgreSql container should be up and running throughout this whole process.

In order to test the data-ingestion script, we will have to drop the table we previously created. Connect to the database using ```pgcli``` and execute the following command:
...
Back in ```pgcli```, list the Tables and see that `yellow_taxi_trips` was created. You can also run a SQL query to check the contents:
...
Let's modify the Dockerfile that we created before to include our `ingest_yellow_taxi_data.py` script and create a new image:
...
# pandas needs Pyarrow for some reason
...
Build the image:
```bash
docker build -t yellow_taxi_data_ingest:v001 .
```
...
* You can drop the table in the database beforehand if you want, but the script will automatically replace the pre-existing table.

## Running Postgres and pgAdmin with Docker-compose

_([Video05 - Running Postgres and pgAdmin with Docker-Compose](https://www.youtube.com/watch?v=hKI6PkPhpa0&list=PL3MmuxUbc_hJed7dXYoJw8DoCuVHhGEQb&index=7))_
...
_([Video06 - SQL Refresher](https://www.youtube.com/watch?v=QEcps_iskgg&ab_channel=DataTalksClub%E2%AC%9B))_
...
Cloud Computing service offered by Google.  Includes a range of hosted services for Compute, Storage, Application Development etc. that run on Google managed infrastructure.
...
_([Video03 - GCP service account setup](https://www.youtube.com/watch?v=Y2ux7gq3Z0o&ab_channel=DataSlinger))_
...
1. Setup a new project and write down the Project ID.
    ...
    1. Give the project a name. You can use the autogenerated Project ID (this ID must be unique to all of GCP, not just your account). Leave the organization as _No organization_. Click on _Create_.
...
1. Setup a service account for this project and download the JSON authentication key files.
    ...
    1. Provide a service account name. Leave all other fields with the default values. Click on _Create and continue_.
...
1. Set the environment variable to point to the auth keys.
    1. The environment variable name is `GOOGLE_CREDENTIALS`
...
We need to setup access first by assigning the _Storage Admin_, _Storage Object Admin_, _BigQuery Admin_ and _Viewer_ IAM roles to the Service Account.
...
1. Make sure that the `GOOGLE_CREDENTIALS` environment variable is set.


## Infrastructure setup with Terraform

_([Video03 - Single file Terraform Deployment](https://www.youtube.com/watch?v=Y2ux7gq3Z0o&ab_channel=DataSlinger))_
...
provider "google" {
  credentials = file("<path/to/authkeys.json>")
...
The infrastructure we will need consists of a Cloud Storage Bucket (`google_storage_bucket`) for our _Data Lake_ and a BigQuery Dataset (`google_bigquery_dataset`).
...
The provider will not make use of the `credentials` field because when we set up GCP access we already created a `GOOGLE_CREDENTIALS` env-var which Terraform can read in order to get our authentication keys.
...
You may access [main.tf](../week1_docker-terraform/1_terraform_gcp/terraform_variables/main.tf) and [variables.tf](../week1_docker-terraform/1_terraform_gcp/terraform_variables/variables.tf). Take a look at them to understand the details of the implementation. Copy them to a new folder within your work directory so that the subfolder only contains the Terraform configuration files. Now run the following commands:
...
Let Terraform access GCP and figure out what to do. The infrastructure plan will be printed on screen with all the planned changes marked with a `+` sign next to them.
...
You will need to confirm this step by typing `yes`/`no` when prompted. On typing `yes`, Terraform will create all the necessary components in the infrastructure an return a `terraform.tfstate` with the current state of the infrastructure.
...
Once again, you will have to confirm this step by typing `yes`/`no` when prompted. On typing `yes`, Terraform will remove your complete stack from the cloud, so only use it when you're 100% sure of it.
...
If you cannot set up a local development environment, you may want to create a Github Codespace (A Remote Development enviornment managed by Github) and access it from your local setup.

[Follow the instructions in this video](https://www.youtube.com/watch?v=XOSUt8Ih3zA&ab_channel=DataTalksClub%E2%AC%9B).
